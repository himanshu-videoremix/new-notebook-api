...
export const api = {
  ...
  async generateDeepDive(topic: string, sources: string[]): Promise<GenerationResponse> {
    try {
      // Validate sources
      if (!sources || sources.length === 0) {
        throw new Error('No sources selected for Deep Dive generation');
      }
      ...
      // Create content through main API endpoint
